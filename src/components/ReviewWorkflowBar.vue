...
const emit = defineEmits<{ run: []; start: []; complete: []; rollbackHuman: []; rollbackAwaiting: []; reload: [] }>()
...
<template>
  <section class="workflow-bar" aria-label="章节复审流程">
    ...
    <template v-else>
      ...
      <div v-else-if="stage==='human_review_in_progress'" class="progress-content"><div class="stats"><span>AI 建议 <b>{{ counts.total }}</b></span><span>待复审 <b>{{ counts.pending }}</b></span><span>已接受 <b>{{ counts.accepted }}</b></span><span>已修改 <b>{{ counts.modified }}</b></span><span>已驳回 <b>{{ counts.rejected }}</b></span><span>人工补充 <b>{{ manualCount }}</b></span></div><div class="complete-row"><span>{{ counts.pending ? `还有 ${counts.pending} 条 AI 建议待复审` : '所有 AI 建议已处理，请确认已完整检查本章。' }}</span><div class="workflow-actions"><button class="secondary" type="button" :disabled="acting||!assigneeName" @click="emit('rollbackAwaiting')">退回 AI 初校结果</button><button type="button" :disabled="acting||counts.pending>0||!assigneeName" @click="emit('complete')">完成本章复审</button></div></div></div>
      <div v-else class="completed-note"><span>✓ 本章复审已完成</span><button class="secondary" type="button" :disabled="acting||!assigneeName" @click="emit('rollbackHuman')">退回人工复审</button></div>
...
<style scoped>
.workflow-bar{padding:11px 14px;color:#56657b;background:#f8f9fb;border-bottom:1px solid #e0e5ec}.workflow-top,.stage-action,.complete-row,.workflow-error,.completed-note{display:flex;align-items:center;justify-content:space-between;gap:10px}.steps{display:flex;align-items:center;gap:6px;color:#a2aab6;font-size:9px;font-weight:700}.steps span{padding:4px 6px;border-radius:4px}.steps span.active{color:#4c6589;background:#e9eef5}.steps b{color:#c0c6cf;font-weight:400}.reviewer{color:#7f8b9c;font-size:9px}.stage-note{display:flex;align-items:baseline;gap:8px;margin-top:9px;font-size:9px}.stage-note strong{color:#3d4e68;font-size:10px}.stage-note span,.stage-action span{color:#8994a4}.error-stage strong{color:#9c5555}.stage-action{margin-top:9px}.stage-action p{display:flex;flex-direction:column;gap:3px;margin:0;font-size:9px}.stage-action strong{color:#40536f;font-size:10px}.stage-action button,.complete-row button,.completed-note button{padding:6px 9px;color:#fff;font-size:9px;background:#4d6d99;border:0;border-radius:5px}.stage-action button:disabled,.complete-row button:disabled,.completed-note button:disabled{cursor:not-allowed;opacity:.45}.progress-content{margin-top:9px}.stats{display:flex;flex-wrap:wrap;gap:5px}.stats span{padding:4px 6px;color:#738094;font-size:8px;background:#eef1f5;border-radius:4px}.stats b{color:#3f587a}.complete-row{margin-top:8px;color:#8a7460;font-size:9px}.workflow-actions{display:flex;gap:7px}.workflow-actions .secondary,.completed-note .secondary{color:#5e6f86;background:#fff;border:1px solid #cfd8e4}.completed-note{margin-top:9px;color:#3f7b66;font-size:11px;font-weight:700}.assignee-warning{margin-top:7px;color:#9b6a45;font-size:9px}.workflow-error{margin-top:7px;padding:6px 7px;color:#a05050;font-size:9px;background:#fff0ee;border-radius:4px}.workflow-error button{padding:0;color:#83506b;background:transparent;border:0;text-decoration:underline}
