...
import type { AiReviewWorkspace } from '../models/aiReview'

const props = defineProps<{ workspace: AiReviewWorkspace | null; assigneeName?: string; manualCount: number; loading: boolean; acting: boolean; error: string; conflict: boolean }>()
const emit = defineEmits<{ start: []; complete: []; reload: [] }>()
...
<template>
  <section class="workflow-bar" aria-label="章节复审流程">
    ...
    <template v-else>
      <div v-if="stage==='awaiting_ai'" class="stage-note"><strong>等待 AI 初校</strong><span>AI 初校尚未运行。人工意见仍可查看和补充。</span></div>
      <div v-else-if="stage==='ai_running'" class="stage-note"><strong>AI 初校进行中</strong><span>人工补充功能仍可使用。</span></div>
      <div v-else-if="stage==='ai_failed'" class="stage-note error-stage"><strong>AI 初校失败</strong><span>等待系统重新执行 AI 初校。</span></div>
