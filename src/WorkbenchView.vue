  ...
  <main class="app-shell">
    ...
    <div class="workspace-grid" :class="{'left-sidebar-collapsed':leftSidebarCollapsed}">
      ...
      <section class="reader-panel" aria-label="PDF 阅读器">
        <div class="reader-toolbar"><div class="reader-title"><span class="reader-file-icon">PDF</span><div><strong>{{ selectedDocument?.title??'尚未上传教材' }}</strong><span>{{ selectedChapter?`本章 PDF ${selectedChapter.startPdfPage}–${selectedChapter.endPdfPage} 页 · ${selectedChapter.title}`:selectedDocument?'整本 PDF · 浏览/章节划分模式':'上传 PDF 后开始校对' }}</span></div></div><div class="reader-hint"><template v-if="annotationsEditable"><span class="shortcut-key">⌘</span>选中文字即可添加高亮或批注</template><span v-else-if="reviewCompleted">本章复审已完成 · PDF 只读 · 如需修改请先退回人工复审</span><span v-else-if="selectedDocument">浏览模式 · 建立或选择章节后开始校对</span></div></div>
        <div class="inklayer-frame">
          <PdfAnnotator v-if="selectedDocument?.processingStatus==='ready'&&!proofreadingLoading" :key="`${selectedDocument.id}:${selectedChapter?.id??'browse'}`" :url="pdfUrl" :user="currentUser" locale="zh-CN" theme="indigo" initial-scale="auto" :layout-style="{width:'100%',height:'100%'}" :default-show-annotations-sidebar="true" :default-show-annotation-author-labels="false" :initial-annotations="inkLayerInitialAnnotations" :actions="annotationsEditable?InkLayerActions:undefined" :annotation-permissions="annotationPermissions" @save="handleSave" @annotation-added="handleAnnotationAdded" @annotation-deleted="handleAnnotationDeleted" @annotation-selected="onAnnotationSelected" @annotation-updated="handleAnnotationUpdated"/>
