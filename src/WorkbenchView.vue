...
<style>
:root{--z-pdf-overlay:30;--z-sidebar-toggle:50;--z-popover:100;--z-drawer:1000;--z-modal-backdrop:2000;--z-modal-dialog:2001}
.chapter-delete-backdrop{position:fixed;inset:0;z-index:var(--z-modal-backdrop);display:grid;place-items:center;padding:20px;background:rgba(23,32,51,.32);pointer-events:auto}
.chapter-delete-dialog{position:relative;z-index:var(--z-modal-dialog);width:min(440px,calc(100vw - 40px));padding:22px;color:#68758b;background:#fff;border:1px solid #dfe4eb;border-radius:11px;box-shadow:0 18px 52px rgba(23,32,51,.22);pointer-events:auto}
