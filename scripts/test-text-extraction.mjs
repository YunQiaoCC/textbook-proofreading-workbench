...
async function main() {
  ...
  try {
    ...
    let app = await createIngestionServer({ storageRoot: root, nativeTextExtractor: fakeExtractor })
    let baseUrl = await listen(app)
    await createDocument(app, 'api-mixed', makePdf(['text', 'image']), 2)
    ...
    assert.equal(completed.nativeTextPages, 1)
    ...
    const recoveredPage = { ...fakeExtracted('recovery', 1), documentId: 'recovery', pdfPage: 1, source: 'pdf_text', status: 'ready', charCount: 70, quality: evaluateNativeTextQuality('Recovered canonical page text remains readable after restart.', { lineCount: 1, wordCount: 8, bboxCount: 1 }), updatedAt: timestamp }
    ...
    await app.textRepository.writeSummary({ id: 'recovery-job', documentId: 'recovery', status: 'processing', totalPages: 3, processedPages: 1, nativeTextPages: 1, ocrRequiredPages: 0, failedPages: 0, createdAt: timestamp, startedAt: timestamp, updatedAt: timestamp })
    await app.close()

    app = await createIngestionServer({ storageRoot: root, nativeTextExtractor: fakeExtractor })
    ...
    await app.close()
    console.log('native-pdf=pdf_text image-only=ocr_required mixed=page1:pdf_text,page2:ocr_required')
